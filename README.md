...
- 邮箱注册、登录、Argon2 密码哈希、JWT、学生/管理员角色和账号启停。
- 管理员专属知识库页面：Markdown/TXT/PDF/DOCX 上传、分页查询、详情、元数据修改、
...
浏览器（React）
  ...
  └─ POST SSE：start → sources → delta → done/error
       ...
       │                                │
MySQL 业务数据库                 独立向量知识库
用户/权限/任务/会话/消息       NPZ 文档工件 → FAISS + BM25
                                       │
...
- 每项事实必须带 `[Sx]` 引用；没有资料时拒答，不使用模型内部记忆补齐。
...
<http://127.0.0.1:8000/api/health>。修改端口可在运行前设置 `PORT` 环境变量。
...
│           ├── DocumentsPage.tsx    管理员知识库上传与 CRUD 页面
