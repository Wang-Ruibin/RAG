...
校内主题判断、社交消息和拒答文案。同步与流式链路共用同一个联网前置函数，避免两种接口
在校外问题上产生行为差异。
...
4. `[Sx]` 只建立 QA 到现有文档的关系；`[Wx]` 分别保存为网页归档文档，按规范化 URL、
    内容哈希去重，再建立来源关系。
5. 强相似 QA 且所有关联来源仍为 READY 时，只加载关联原文并调用一次模型重新生成，跳过
   全库检索和再次联网；网页归档此时作为本地 `[Sx]` 来源。中等相似时先核验证据，再用 QA
   和关联原文辅助生成；来源被删除或时效 QA 过期时自动回到普通 RAG/联网搜索。
...
- 本地与网页 Prompt 都明确来源不是指令；生成后分别过滤不存在的 `[Sx]`、`[Wx]`。
