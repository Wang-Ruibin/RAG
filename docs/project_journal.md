...
### 2026-07-18：QA 证据降级与引用连续化

- 修复实时网页只引用原 W1、W2、W5 时，来源卡数量为 3 但正文仍保留 W5 的编号错位：
  本地、网页、混合和 QA 四类回答统一在过滤无效/未引用来源后，分别把 S/W 编号压缩为
  连续序号，来源对象的 `citation_index` 同步更新。
- `sources` SSE 最终事件新增清洗后的完整 `answer`；前端用它覆盖逐 token 阶段的正文，
  因此无需刷新历史会话也能立即看到连续编号，持久化内容与当前页面保持一致。
- 直接命中和辅助命中的隐藏 QA 统一核验当前关联原文是否足以完整回答。即使 QA 相似度很
  高，只要网页归档只有标题/链接等不完整内容，也不能直接依赖历史答案生成。
- QA 证据不足时不再直接联网，而是先执行完整 Dense/BM25/RRF/Rerank 检索；完整 RAG
  仍不能回答时，再判断是否属于河海大学，校内问题联网补充，校外问题礼貌拒答。同步和
  SSE 两条链路使用相同顺序，最终 `answer_origin` 按实际留下的来源重新计算。
- 紧邻本次文档更新之前的验证基线为 Ruff、后端 59/59、前端 11/11、ESLint 和生产构建
  全部通过；生产式重启后健康检查正常。当前只读统计为 1,041 篇 READY 文档（1,027
  普通知识 + 14 网页归档）、3,895 chunk、5 条启用隐藏 QA、0 篇人工纠错文档。
- 本次提交仅同步说明文档，按用户要求不重复运行测试；历史 1,026/3,867 检索评测结果
  继续作为固定基线，不用当前运行时数量冒充重新评测。
