...
- 原始知识库：`knowledge_docs/` 共 1,026 篇河海大学相关 Markdown，约 5.5 MB。
...
6. 检索采用向量 Top20 + BM25 Top20 + RRF Top12 + Rerank Top5，兼顾语义问题与
   教室编号、部门名、政策名称等精确关键词。
7. 上传立即返回 202，单工作线程后台解析/切块/向量化；索引锁不覆盖 LLM 流式生成。
8. 每项事实要求 `[Sx]` 引用，资料不足明确拒答，来源卡展示标题、日期、URL、分数和片段。
