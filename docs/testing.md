...
- 单元：配置 URL 安全组装、解析/清洗、标题路径切块、RRF、引用过滤与稀疏编号连续化、
  免费/百度搜索规范化与错误分类。
- 接口：注册登录、管理员权限、自停用保护、会话越权、文档只读/写权限、预览下载、
  运行时删除、QA 去重/归档、QA 证据不足时完整 RAG→联网降级、纠错审批和校外问题免联网。
- 前端：SSE 分帧解析、最终回答覆盖流式旧编号、引用折叠与颜色、主动欢迎语、
  复制/点赞/倒赞、普通用户只读知识库、TypeScript、ESLint、Vitest、Vite 生产构建。
...
2026-07-18 最近一次自动化基线：后端 Ruff 通过、59 个 pytest 全部通过；前端 ESLint、
11 个 Vitest 和生产构建通过。测试覆盖热上传与运行时删除、边界词法命中、上下文弱相关
过滤、稀疏 W 引用连续化、最终回答 SSE 覆盖、知识库分页/预览/下载、QA 快速命中及去重、
QA 关联资料不足时同步/流式链路都先执行完整 RAG 再联网、网页归档、纠错审批、引用颜色、
主动欢迎语、历史会话标题搜索/重命名/越权保护，以及校外问题不触发网页搜索等回归场景。

本次仅更新说明文档，未重复运行测试；上述 59/11 是紧邻本次文档更新之前的已通过基线。
...
`计算题.txt`、网页归档和隐藏 QA，因此原始评测 JSON 不随当前 1,041/3,895 本机运行
状态改写。最终精排参数为 5 个候选、最大长度 256；独立 FAISS 知识库重构后
