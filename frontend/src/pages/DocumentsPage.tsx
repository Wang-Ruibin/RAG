...
import { Alert, Button, Card, Descriptions, Drawer, Form, Input, Modal, Popconfirm, Progress, Space, Table, Tag, Typography, Upload, message } from 'antd'
...
export function DocumentsPage() {
  ...
  async function showDetail(id: number) {
    const detail = await api<CampusDocument>(`/api/documents/${id}`)
    setViewing(detail)
  }
  ...
  return (
    <div className="page-container">
      ...
      <Card>
        <Table
          ...
          columns={[
            ...
            { title: '操作', fixed: 'right', width: 180, render: (_, row) => <Space>
              <Button type="text" icon={<EyeOutlined />} onClick={() => void showDetail(row.id)} />
              <Button type="text" icon={<EditOutlined />} disabled={['QUEUED', 'PROCESSING'].includes(row.status)} onClick={() => startEdit(row)} />
              <Button type="text" icon={<ReloadOutlined />} disabled={['QUEUED', 'PROCESSING'].includes(row.status)} onClick={() => void reindex(row.id)} />
              <Popconfirm title="同时删除原文件和知识库中的文本块、向量？" onConfirm={() => void remove(row.id)}><Button type="text" danger icon={<DeleteOutlined />} /></Popconfirm>
            </Space> },
          ]}
        />
      </Card>
      ...
      <Drawer title="知识库资料详情" width={560} open={Boolean(viewing)} onClose={() => setViewing(null)}>
