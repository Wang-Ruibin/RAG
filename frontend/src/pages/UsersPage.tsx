...
export function UsersPage() {
  ...
  return (
    <div className="page-container">
      <header className="page-header"><div><Typography.Title level={3}>用户管理</Typography.Title><Typography.Text type="secondary">管理学生账号状态和管理员角色</Typography.Text></div></header>
      <Card>
        <Table
          ...
          columns={[
            ...
            { title: '角色', dataIndex: 'role', render: (role: Role, row) => row.id === current?.id ? <Tag color="purple">当前管理员</Tag> : <Select value={role} style={{ width: 110 }} options={[{ value: 'STUDENT', label: '学生' }, { value: 'ADMIN', label: '管理员' }]} onChange={(value) => void update(row.id, { role: value })} /> },
